...
export function StatusBadge({ status, label, className = '' }: StatusBadgeProps) {
  ...
  if (['active', 'paid', 'completed'].includes(normalized)) {
    styleClasses = 'bg-success text-success-text border-emerald-200'
  }
  // Warning
  else if (['expiring', 'expiring_soon', 'partial', 'pending'].includes(normalized)) {
    styleClasses = 'bg-warning text-warning-text border-amber-200'
  }
  // Danger
  else if (['expired', 'failed', 'overdue', 'no_show', 'broken'].includes(normalized)) {
    styleClasses = 'bg-danger text-danger-text border-red-200'
  ...
  // Info
    ...
    styleClasses = 'bg-info text-info-text border-sky-200'
  }
  // Neutral
  else if (['inactive', 'archived', 'cancelled', 'refunded', 'retired'].includes(normalized)) {
    styleClasses = 'bg-neutralBadge text-neutralBadge-text border-slate-200'
  }
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold border ${styleClasses} ${className}`}
    >
